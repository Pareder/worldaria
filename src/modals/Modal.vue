        ...
        <div v-else>
          ...
          <div v-if="!this.$route.params.sort">
            ...
            <div class="leaderboard">
              <h1>Leaderboard</h1>
              <ol>
                <li v-for="(user, id) in reverseLeaders(leaderboard)" :key="id" :class="user['.key'] === hash ? 'leader' : ''">
                  <mark>{{ user.name }}</mark>
                  <span>{{ user.score }} | {{ user.scoreDate ? new Date(user.scoreDate).toLocaleDateString() : '' }}</span>
                </li>
              </ol>
            </div>
          </div>
...
export default {
  ...
  created () {
    if (this.getCookie('name')) {
      let cookieInfo = JSON.parse(this.getCookie('name'))
      this.hash = cookieInfo.hash
      this.nickname = cookieInfo.name
      console.log(this.nickname, cookieInfo)
    }
    if (this.mode === 'game' && !this.$route.params.sort) {
      if (this.hash) {
        let dbScore = USERS.child(this.hash + '/score')
