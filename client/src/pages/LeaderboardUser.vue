<template>
  <v-row no-gutters align="center" class="mb-5">
    <v-avatar :color="stringToColor(user)">
...
import type { LeaderboardUserType } from '@/types'
import { firestore } from '@/config'
import stringToColor from '@/utils/stringToColor'
import UserRecords from '@/components/UserRecords'

const route = useRoute()
const data = ref<LeaderboardUserType>()
...
onMounted(async () => {
  ...
  if (snapshot.exists()) {
    data.value = snapshot.data() as LeaderboardUserType
  }
})
</script>
