...
class Cron {
  ...
  makeTask(date) {
    ...
    const mailTask = this._nodeCron.schedule(`0 0 ${new Date(dayBefore).getDate()} ${new Date(date).getMonth() + 1} *`, () => {
    ...
    const readApiTask = this._nodeCron.schedule(`0 0 ${new Date(date).getDate()} ${new Date(date).getMonth() + 1} *`, () => {
